...
# Track active tab index using a hidden radio (Streamlit executes all with blocks, so we must check index)
if "active_tab" not in st.session_state:
    st.session_state["active_tab"] = 0
active_tab = st.radio("_active_tab", tab_labels, index=st.session_state["active_tab"], label_visibility="collapsed")
st.session_state["active_tab"] = tab_labels.index(active_tab)

for i, tab in enumerate(tabs):
    with tab:
        if st.session_state["active_tab"] != i:
            continue  # Only run code for the active tab
        if i == 0:
